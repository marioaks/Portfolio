const ragAdjust = function (s, method) {
	if (document.querySelectorAll) {
		const eles = document.querySelectorAll(s);
		let elescount = eles.length;

		const preps = /(\s|^|>)((aboard|about|above|across|after|against|along|amid|among|anti|around|before|behind|below|beneath|beside|besides|between|beyond|concerning|considering|despite|down|during|except|excepting|excluding|following|from|inside|into|like|minus|near|onto|opposite|outside|over|past|plus|regarding|round|save|since|than|that|this|through|toward|towards|under|underneath|unlike|until|upon|versus|with|within|without)\s)+/gi;

		const smallwords = /(\s|^)(([a-zA-Z-_(]{1,2}('|’)*[a-zA-Z-_,;]{0,1}?\s)+)/gi; // words with 3 or less characters

		const dashes = /([-–—])\s/gi;

		const emphasis = /(<(strong|em|b|i)>)(([^\s]+\s*){2,3})?(<\/(strong|em|b|i)>)/gi;

		while (elescount-- > 0) {
			const ele = eles[elescount];
