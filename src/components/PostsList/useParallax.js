...
export default function useParallax() {
	useEffect(() => {
		...
		let projectsContainer = document.querySelector('#projects-wrapper')
		let projects = document.querySelectorAll('.project')

		function animateItems() {
			let origin = `${windowCenter[0]}px ${windowCenter[1]}px`,
			...
			while (i--) {
				...
				const delta = mojs.helpers.clamp(inEasing(a), 0.03, 1),
					  ...
					  deltaShift = mojs.h.clamp(inEasing(a), 0.03, 1),
					  isDeltaChanged = projectDiv.prevDelta !== delta,
					  isDeltaShiftChanged = projectDiv.prevDeltaShift !== deltaShift;
		    ...
		    return () => { cancelAnimationFrame(animateItems) }
		}

		const unsubscribeAnimateItems = animateItems()

		return () => { unsubscribeAnimateItems() }
	}, [])
}