...
subtitle: Using machine learning to explore the ways that neighborhoods are connected by live music. I was able to set up a collaborative filtering model with Scikit Learn to explore neighborhood, venue, and artist similarities based on touring patterns accross the US. I then used D3 to visualize the results for the following article. A special thanks to Will Shapiro for the writeup. 
